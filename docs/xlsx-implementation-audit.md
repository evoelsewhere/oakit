...
| Typed edits, R1, and R2                               | Partial  | Public `applyXlsxEdits`/JSON validation/write support constrained `set-cell`, `clear-cell`, normalized `set-cell-style`, and internal-target `set-hyperlink` at `verified-R2`. Ordered atomic replay enforces exact shapes, unique IDs, canonical `ifMatch`, stable sheet keys, aggregate budgets, merged-cell and multi-cell-hyperlink guards, isolated previews, and state hashes. Style edits reuse existing semantic styles or append new font/fill/border/number-format/cell-XF records deterministically without renumbering source IDs; repeated new styles deduplicate in operation order. Hyperlink edits update, append, or remove internal locations while preserving authored display/tooltip metadata and remaining hyperlink order; external URL inputs are protocol/credential/canonical-form validated but explicitly blocked at write until relationship-part allocation is implemented. Local UTF-8/UTF-16 patches preserve prefixes, untouched markup, source ZIP dates, and formula/cache/value bytes, with exact aggregate XML/patch budgets and R1 part classification. Fresh strict R2 reparse compares the complete keyed document. Transitional/Strict, scalar/formula/blank/style/internal-hyperlink, two-sheet ownership, concurrency, security, exact resource boundaries, and direct verifier tests pass; current aggregate mutation is 100% with 0 survivor/no-coverage/timeout (`style-append.ts`: 323 killed/131 compile errors; `style-validation.ts`: 357/103; `hyperlink-patch.ts`: 217/54).                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | This row remains partial because external hyperlink relationship allocation and the remaining typed operations (rows/columns, structural sheets, tables/rules, comments, drawings/images, charts, pivots, checkbox feature-graph creation, and other planned domains) still need their own impact closure and R1/R2 evidence; R3 producer evidence is also pending. |
...
dependency has been added. The style/hyperlink slices add only their three XLSX
modules to the shared `scripts/mutation-scope.mjs` CI registry; they do not
change, exclude, or narrow any PPTX mutation target.
