...
| Drawing anchors and embedded images                   | Partial  | `drawing.ts` resolves worksheet-owned drawing parts and drawing-owned embedded images, shapes, connectors, recursive groups, and chart graphic frames. The portable object union preserves stable authored IDs/names/descriptions/hidden flags, preset/custom geometry, solid/no-fill colors, lines, ordered text/runs/fields/breaks, signed object/group/chart transforms, connector endpoints, group child coordinate spaces, and authored nested-object order. `chart.ts` follows owner-scoped internal relationships and normalizes all declared common ChartML families, multi-plot authored order, series identities, literal/reference number/string/multi-level sources, bounded formulas and caches, axes/crossing/scaling/number formats/gridlines, titles, legends, labels, colors, markers, styles, blank-display policy, and plot options without evaluating formulas or fetching external data. Anchors retain ordered absolute/one-cell/two-cell geometry, edit modes, crop, safe MIME metadata, selection relation, and all explicit image modes. Media reads and repeated returned bytes are bounded; external/SVG images and external charts are rejected; failed optional or fatal parses roll back media state. Transitional/Strict/unprefixed namespaces, portable JSON/exact R0, duplicate IDs, malformed/spoofed roots, relationship/content-type ownership, exact chart/drawing/formula/text limits, nested charts, and tolerant/strict recovery pass. Full mutation evidence is 100%: `chart.ts` 594 killed, `drawing.ts` 601 killed, parser 116 killed, zero survivor, no-coverage, or timeout. | Sparkline objects remain missing, so the broader visual-feature row is not complete. Drawing/image/chart operations, stable object keys, R1/R2, browser lifecycle CI, producer corpus and renderer evidence remain pending.                                                                             |
| Reader range/visual/analytical features               | Partial  | Tables, worksheet/table filters and sorts, data validations, base SpreadsheetML conditional formatting, DrawingML images/shapes/connectors/groups, and common ChartML families now have typed portable models. Conditional formatting preserves authored container/rule order, globally unique priorities, dxf references, formulas, text/time/average/rank rules, all comparison operators, six threshold kinds, colors, color scales, data bars, icon sets, pivots/default flags, multi-ranges, and selection relation. Charts preserve series formulas/caches, axes, titles, legends, colors, styles, labels, and family-specific options. These domains enforce workbook-wide rule and exact range/text/formula/grid/drawing/chart/work limits and have Strict plus byte-identical JSON-cloned all-copy R0 fixtures. Public valid/invalid/selection/ownership/boundary tests pass; current module/drawing/worksheet/parser mutation evidence is 100% with zero survivor, no-coverage, or timeout.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | Sparklines, pivots/caches/slicers/timelines, x14 conditional/validation extensions and other known extensions, producer corpus, and portable rendering follow the remaining vertical-slice path.                                                                                                        |
