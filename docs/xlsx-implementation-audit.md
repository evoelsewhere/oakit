...
| Typed edits, R1, and R2                               | Partial  | Typed operation discriminants and an honest `unsupported` public capability surface exist. Internal `set-cell`/`clear-cell` validation and pure replay now enforce exact operation shapes, unique IDs, canonical `ifMatch`, stable sheet keys, ordered atomic semantics, scalar/formula/text budgets, merged-cell guards, isolated previews, state hashes, and focused mutation 100%.                                                     | The planner is intentionally not wired to the public editor or advertised yet; package impact closure, deterministic XML patching, R1 part evidence, output reopen, and independent R2 semantic verification remain before capability promotion. |
