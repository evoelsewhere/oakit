...
| Typed edits, R1, and R2                               | Partial  | Public `applyXlsxEdits`/JSON validation/write support constrained `set-cell`, `clear-cell`, normalized `set-cell-style`, internal/external `set-hyperlink`, and non-structural `set-row`/`set-column` at `verified-R2`. Ordered atomic replay enforces exact shapes, unique IDs, canonical target-specific `ifMatch`, stable sheet keys, aggregate budgets, isolated previews, and state hashes. Style edits reuse or append deterministic records. Hyperlinks use owner-scoped deterministic relationship allocation. Row/column edits set size and visibility only on existing explicit rows or exact authored normalized column ranges; local UTF-8/UTF-16 patches preserve cells, formulas, styles, outline metadata, prefixes, Strict/Transitional dialects, unrelated attributes, and source ZIP dates. R1 classifies copied, patched, and added parts literally; fresh strict R2 reparse compares the complete keyed document. Direct XML adversarial tests cover nested/foreign/sibling lookalikes, missing/ambiguous spans, ordering, exact resource bounds, and mixed-operation aggregate accounting. Current mutation evidence is 100% with 0 survivor/no-coverage/timeout (`worksheet-properties-patch.ts`: 180 killed/55 compile errors; `operation-planner.ts`: 172/138; `operation-validation.ts`: 434/320; `cell-edit-package.ts`: 86/86; aggregate: 16,057 killed/9,809 compile errors).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | This row remains partial because structural row/column reference transforms and the remaining typed operations (structural sheets, tables/rules, comments, drawings/images, charts, pivots, checkbox feature-graph creation, and other planned domains) still need their own impact closure and R1/R2 evidence; R3 producer evidence is also pending. |
...
dependency has been added. The style/hyperlink/row-column slices add only their five XLSX
