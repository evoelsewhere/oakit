...
| Drawing anchors and embedded images                   | Partial  | `drawing.ts` resolves worksheet-owned drawing parts and drawing-owned embedded images with ordered absolute/one-cell/two-cell anchors, 1-based cell markers, point offsets/extents/positions, edit modes, transforms, crop, safe MIME metadata, selection relation, and all explicit image modes. Media reads and repeated returned bytes are bounded; external/SVG images are rejected; bytes and object URLs are parse-local and deduplicated; failed optional or fatal parses roll back cache/accounting and revoke parser-owned URLs. Transitional/Strict, portable JSON/exact R0, relationship/content-type ownership, Base64/blob/both/none, caller ownership, interleaved order, geometry/property boundaries, and resource tests pass. `drawing.ts`, part-reader presence checks, worksheet capture, parser orchestration and cleanup have 100% mutation evidence with zero survivor/no-coverage/timeout.             | DrawingML shapes/groups/connectors and chart/sparkline objects remain missing, so the broader drawings/images row is not complete. Image operations, stable object keys, R1/R2, browser lifecycle CI, producer corpus and renderer evidence remain pending.                                             |
| Reader range/visual/analytical features               | Partial  | Tables, worksheet/table filters and sorts, data validations, and base SpreadsheetML conditional formatting now have typed portable models. Conditional formatting preserves authored container/rule order, globally unique priorities, dxf references, formulas, text/time/average/rank rules, all comparison operators, six threshold kinds, colors, color scales, data bars, icon sets, pivots/default flags, multi-ranges, and selection relation. These domains enforce workbook-wide rule and exact range/text/formula/grid/work limits and have Strict plus byte-identical JSON-cloned all-copy R0 fixtures. Public valid/invalid/selection/ownership/boundary tests pass; current module/worksheet/parser mutation evidence is 100% with zero survivor, no-coverage, or timeout.                                                                                                                                       | DrawingML shapes/groups/connectors, charts/sparklines, pivots/caches/slicers/timelines, x14 conditional/validation extensions and other known extensions, producer corpus, and portable rendering follow the remaining vertical-slice path.                                                             |
