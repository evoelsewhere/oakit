...
| Typed edits, R1, and R2                               | Partial  | Public `applyXlsxEdits`/JSON validation/write support constrained `set-cell`, `clear-cell`, and normalized `set-cell-style` at `verified-R2`. Ordered atomic replay enforces exact shapes, unique IDs, canonical `ifMatch`, stable sheet keys, aggregate budgets, merged-cell guards, isolated previews, and state hashes. Style edits reuse existing semantic styles or append new font/fill/border/number-format/cell-XF records deterministically without renumbering source IDs; repeated new styles deduplicate in operation order. Local UTF-8/UTF-16 style patches preserve prefixes, untouched markup, source ZIP dates, and formula/cache/value bytes, classify both worksheet and styles parts in R1, and reject number/date interpretation changes, missing styles parts, exhausted IDs, and new checkbox styles that require a feature-property-bag edit. Fresh strict R2 reparse compares the complete keyed document. Transitional/Strict, scalar/formula/blank/style, two-sheet, concurrency, security, exact resource boundaries, and direct verifier tests pass; current aggregate mutation is 100% with 0 survivor/no-coverage/timeout (`style-append.ts`: 323 killed/131 compile errors; `style-validation.ts`: 357/103).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | This row remains partial because the remaining typed operations (hyperlinks, rows/columns, structural sheets, tables/rules, comments, drawings/images, charts, pivots, checkbox feature-graph creation, and other planned domains) still need their own impact closure and R1/R2 evidence; R3 producer evidence is also pending. |
...
dependency has been added. This style-append slice adds only its two XLSX
modules to the shared `scripts/mutation-scope.mjs` CI registry; it does not
