...
| Reader range/visual/analytical features               | Partial  | Worksheet and table-owned auto-filters preserve value/date/custom/dynamic/top/color/icon rules, custom lists, and authored sort state without applying filtering. SpreadsheetML tables resolve owner-scoped Strict/Transitional relationships and content types, enforce workbook-global identities and exact table/range/text/formula limits, and expose table types, connection/comment metadata, ranges, headers/totals, formulas, named and differential styles, query fields, style flags, and selection relation as portable JSON. A JSON-cloned table/filter snapshot returns byte-identical all-copy R0 output. Public valid/invalid/selection/ownership/boundary tests pass, and current auto-filter/table/worksheet/parser mutation evidence is 100% with zero survivor, no-coverage, or timeout.                                                                                                                 | Data validation, conditional formatting, comments, print/protection, drawings/images/charts/sparklines, pivots/caches/slicers/timelines, known extensions, producer corpus, and portable rendering follow the remaining vertical-slice path.                                                            |
