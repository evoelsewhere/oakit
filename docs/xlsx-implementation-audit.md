...
| Typed edits, R1, and R2                               | Partial  | Public `applyXlsxEdits`/JSON validation/write now support constrained `set-cell`, `clear-cell`, and existing-normalized-style `set-cell-style` at `verified-R2`. Ordered atomic replay enforces exact shapes, unique IDs, canonical `ifMatch`, stable sheet keys, aggregate budgets, merged-cell guards, isolated previews, and state hashes. Style edits deterministically resolve the authored cell-XF index, replace only the cell `s` attribute while preserving formula/cache/value bytes, keep the styles part byte-identical, and reject number/date interpretation changes. The writer maps owner sheet parts, blocks active/signed/opaque/unknown/formula/date/group conflicts, preserves UTF-8/UTF-16/BOM/prefix/allowed attributes and authored ZIP dates, emits bounded deterministic ZIP, reopens the package graph, proves every `copy` part by uncompressed hash/length, reports `patch` parts, then independently reparses and compares the full keyed semantic document. Transitional/Strict, scalar/formula/blank/style, safe external hyperlink, two-sheet, concurrency, security, R1/R2, and direct verifier tests pass; current aggregate mutation is 100% with 0 survivor/no-coverage/timeout.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            | This row remains partial because style append/rebuild and the remaining typed operations (hyperlinks, rows/columns, structural sheets, tables/rules, comments, drawings/images, charts, pivots, and other planned domains) still need their own impact closure and R1/R2 evidence; R3 producer evidence is also pending. |
