...
| Typed edits, R1, and R2                               | Partial  | Public `applyXlsxEdits`/JSON validation/write supports constrained cell/style/hyperlink operations, non-structural row/column properties, and `insert-rows`/`delete-rows`/`insert-columns`/`delete-columns` for a conservative worksheet profile at `verified-R2`. Structural replay validates exact grid-bound shapes, whole-sheet `ifMatch`, atomic structural-only batches, reference budgets, and overflow. A shared coordinate/range transform shifts, expands, clips, or removes authored cells, declared dimensions, merged ranges, internal/external hyperlink ranges, worksheet auto-filter ranges, sort-state ranges, sort-condition ranges, formula-free data-validation multi-ranges, formula-free conditional-format multi-ranges, named protected multi-ranges, and row/column page-break positions plus zero-based extents consistently in preview and prefixed XML. Full-grid break defaults remain bounded; authored bounds, exact count/manual-count attributes, and empty collections update deterministically without changing unrelated margins, page setup, options, or headers/footers. It preserves literal content/styles/unrelated attributes/encodings/ZIP dates and blocks formulas, names, calc chains, tables, formula-bearing validations and conditional formats (including formula `cfvo` thresholds), comments, drawings, sheet-protection/views, analytical metadata, cell metadata, and explicit column definitions with named feature classes. R1 reports literal parts and fresh strict R2 compares the complete keyed document. Transitional/Strict, XML ownership/depth/order traps, every insert/delete/range boundary, exact resource edges, and deterministic writes pass without Office software. Current mutation evidence is 100% with 0 survivor/no-coverage/timeout (`structural-reference.ts`: 116 killed/59 compile errors; `worksheet-structure-patch.ts`: 759/266; `operation-planner.ts`: 461/270; `operation-validation.ts`: 457/341; aggregate: 17,266 killed/10,313 compile errors). | This row remains partial because formula-aware and remaining feature-range structural transforms beyond the conservative profile, plus the remaining typed operations (structural sheets, tables/rules, comments, drawings/images, charts, pivots, checkbox feature-graph creation, and other planned domains), still need impact closure and R1/R2 evidence; R3 producer evidence is also pending. |
