...
| Typed edits, R1, and R2                               | Partial  | Public `applyXlsxEdits`/JSON validation/write support constrained `set-cell`, `clear-cell`, normalized `set-cell-style`, and internal/external `set-hyperlink` at `verified-R2`. Ordered atomic replay enforces exact shapes, unique IDs, canonical `ifMatch`, stable sheet keys, aggregate budgets, merged-cell and multi-cell-hyperlink guards, isolated previews, and state hashes. Style edits reuse existing semantic styles or append deterministic style records without renumbering source IDs. Hyperlink edits update, append, or remove internal locations and allowlisted canonical external URLs, preserve display/tooltip metadata and unrelated relationship nodes, reuse owned hyperlink IDs, append deterministic `rIdN` values, create or patch worksheet relationship parts, and preserve Strict/Transitional relationship dialects. R1 now classifies copied, patched, and added parts literally while allowing only declared owner-scoped relationship closure; fresh strict R2 reparse compares the complete keyed document. Exact resource/security boundaries, UTF-8/UTF-16 XML, two-sheet ownership, mixed cell/hyperlink patch accounting, concurrency, Strict fixtures, and deterministic repeated writes pass. Current mutation evidence is 100% with 0 survivor/no-coverage/timeout (`style-append.ts`: 323 killed/131 compile errors; `style-validation.ts`: 357/103; `hyperlink-patch.ts`: 241/63; `hyperlink-relationships.ts`: 156/70; `cell-edit-verification.ts`: 80/37; aggregate: 15,741 killed/9,589 compile errors).                                                                                                                                                                                                                                                                                                                                                                                                                                      | This row remains partial because the remaining typed operations (rows/columns, structural sheets, tables/rules, comments, drawings/images, charts, pivots, checkbox feature-graph creation, and other planned domains) still need their own impact closure and R1/R2 evidence; R3 producer evidence is also pending. |
...
dependency has been added. The style/hyperlink slices add only their four XLSX
