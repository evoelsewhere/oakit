...
| Typed edits, R1, and R2                               | Partial  | Typed operation discriminants and an honest `unsupported` public capability surface exist. Internal `set-cell`/`clear-cell` validation and pure replay enforce exact shapes, unique IDs, canonical `ifMatch`, stable sheet keys, ordered atomic semantics, budgets, merged-cell guards, isolated previews, and state hashes. `worksheet-patch.ts` now records bounded target token spans, preserves UTF-8/UTF-16/BOM/prefix/allowed attributes, escapes XML 1.0, blocks grouped formulas/extensions, applies non-overlapping patches, and has focused mutation 100% with 0 survivor/no-coverage/timeout. | The planner and patcher are intentionally not wired to the public editor or advertised yet; package impact closure, deterministic ZIP writing, R1 part evidence, output reopen, and independent R2 semantic verification remain before capability promotion. |
