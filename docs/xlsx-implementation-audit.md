...
| Typed edits, R1, and R2                               | Partial  | Public `applyXlsxEdits`/JSON validation/write supports constrained cell/style/hyperlink operations, non-structural row/column properties, and `insert-rows`/`delete-rows`/`insert-columns`/`delete-columns` for reference-free simple worksheets at `verified-R2`. Structural replay validates exact grid-bound index/count shapes, whole-sheet `ifMatch`, structural-only atomic batches, reference-update budgets, and grid overflow. It shifts/removes authored row and cell identities in operation order, removes stale row spans for column shifts, preserves literal content/styles/unrelated attributes/encodings/prefixes/ZIP dates, and blocks every formula, name, calc-chain, range, table/rule, comment, drawing, hyperlink, print/protection/view, pivot/query/slicer/timeline, metadata, declared-dimension, and explicit-column dependency with a named structured feature class. R1 reports literal copied/patched/added parts; fresh strict R2 reparse compares the complete keyed document. Transitional, prefixed Strict, XML depth/order traps, every insert/delete boundary, max-reference/grid/resource edges, and deterministic repeated writes pass without Office software. Current mutation evidence is 100% with 0 survivor/no-coverage/timeout (`worksheet-structure-patch.ts`: 171 killed/57 compile errors; `worksheet-properties-patch.ts`: 180/55; `operation-planner.ts`: 398/190; `operation-validation.ts`: 457/341; `cell-edit-package.ts`: 93/108; aggregate: 16,499 killed/9,965 compile errors).                                                                                                                                                                                                                                                                                                                                                                                                                                                        | This row remains partial because formula/range-aware structural transforms beyond the explicit reference-free profile and the remaining typed operations (structural sheets, tables/rules, comments, drawings/images, charts, pivots, checkbox feature-graph creation, and other planned domains) still need their own impact closure and R1/R2 evidence; R3 producer evidence is also pending. |
...
dependency has been added. The style/hyperlink/row-column slices add only their six XLSX
