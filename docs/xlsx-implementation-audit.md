...
| Drawing anchors and embedded images                   | Partial  | `drawing.ts` resolves worksheet-owned drawing parts and drawing-owned embedded images, shapes, connectors, and recursive groups. The portable object union preserves stable authored IDs/names/descriptions/hidden flags, preset/custom geometry, solid/no-fill colors, lines, ordered text/runs/fields/breaks, signed object/group transforms, connector endpoints, group child coordinate spaces, and authored nested-object order. Anchors retain ordered absolute/one-cell/two-cell geometry, edit modes, crop, safe MIME metadata, selection relation, and all explicit image modes. Media reads and repeated returned bytes are bounded; external/SVG images are rejected; bytes and object URLs are parse-local and deduplicated; failed optional or fatal parses roll back cache/accounting and revoke parser-owned URLs. Transitional/Strict, portable JSON/exact R0, duplicate IDs, malformed object contracts, exact text/drawing limits, relationship/content-type ownership, Base64/blob/both/none, caller ownership, and resource tests pass. Full-module mutation evidence is 100%: 567 killed, zero survivor, no-coverage, or timeout. | Chart graphic frames and sparkline objects remain missing, so the broader visual-feature row is not complete. Drawing/image operations, stable object keys, R1/R2, browser lifecycle CI, producer corpus and renderer evidence remain pending.                                                          |
| Reader range/visual/analytical features               | Partial  | Tables, worksheet/table filters and sorts, data validations, base SpreadsheetML conditional formatting, and DrawingML images/shapes/connectors/groups now have typed portable models. Conditional formatting preserves authored container/rule order, globally unique priorities, dxf references, formulas, text/time/average/rank rules, all comparison operators, six threshold kinds, colors, color scales, data bars, icon sets, pivots/default flags, multi-ranges, and selection relation. These domains enforce workbook-wide rule and exact range/text/formula/grid/drawing/work limits and have Strict plus byte-identical JSON-cloned all-copy R0 fixtures. Public valid/invalid/selection/ownership/boundary tests pass; current module/worksheet/parser mutation evidence is 100% with zero survivor, no-coverage, or timeout.                                                                                                                                                                                                                                                                                                             | Charts/sparklines, pivots/caches/slicers/timelines, x14 conditional/validation extensions and other known extensions, producer corpus, and portable rendering follow the remaining vertical-slice path.                                                                                                 |
