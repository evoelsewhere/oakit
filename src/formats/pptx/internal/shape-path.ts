...
function getGuideAttribute(
  guide: XmlLookupValue | undefined,
  attribute: string,
): string | undefined {
  return getTextByPathList<string>(guide, ['attrs', attribute]);
}
...
export function getShapePath(
  ...
  switch (shapType) {
    ...
    case 'flowChartPunchedCard':
      {
        const shapAdjst_ary = getTextByPathList(node, [
          'p:spPr',
          'a:prstGeom',
          'a:avLst',
          'a:gd',
    ...
    case 'leftBrace':
      {
        ...
        const cd2 = 180,
          cd4 = 90,
          c3d4 = 270;
        const a2 = Math.min(Math.max(adj2, 0), cnstVal2);
        const minWH = Math.min(w, h);
    ...
    case 'leftBracket':
      {
        ...
        const cd2 = 180,
          ...
          c3d4 = 270;
        const a = Math.min(Math.max(adj, 0), maxAdj);
        const y1 = Math.min((Math.min(w, h) * a) / cnstVal2, w);
        const y2 = h - y1;
    ...
    case 'cloudCallout':
      {
        ...
        let lastPoint: [number, number] = [x0, y0];
    ...
    case 'quadArrowCallout':
      {
        ...
        const q2 = a3 * 2;
        const maxAdj4 = cnstVal2 - q2;
        const a4 = Math.min(Math.max(adj4, a1), maxAdj4);
    ...
    case 'circularArrow':
      {
        ...
        const aAng = Math.min(Math.max(toRadians(adj2), 0), maxAng);
        ...
        const istAng = normalizeAngle(ist0);
        const isw1 = stAng - istAng;
        const iswAng = normalizeNegativeAngle(isw1);
        ...
        const sw0 = en2 - stAng;
        const swAng = normalizeAngle(sw0);
